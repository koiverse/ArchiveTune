...
actual object TTMLParser {
    ...
    actual fun parseTTML(ttml: String): List<ParsedLine> {
            ...
            for (divIdx in 0 until divElements.length) {
                ...
                for (pIdx in 0 until pElements.length) {
                    ...
                    if (words.isEmpty() && lineText.isNotEmpty()) {
                        generateFallbackWords(lineText.toString(), startTime, endTime, words)
                    } else if (lineText.isEmpty()) {
                        val directText = getDirectTextContent(pElement).trim()
                        if (directText.isNotEmpty()) {
                            lineText.append(directText)
                            generateFallbackWords(directText, startTime, endTime, words)
                        }
                    }
    ...
    private fun generateFallbackWords(
        text: String,
        startTime: Double,
        endTime: Double,
